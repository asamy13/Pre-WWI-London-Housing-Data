...
proc import datafile="<Insert-Folder-Path>\hpi_lonmart_imputed.csv"
			out=lonmart_data
			replace;
run;
...
data lonmart_data;
	set lonmart_data;
	if 1895 <= year <= 1914 and 100 <= value <= 1000 and years_purchase >= 2.8;
run;
...
proc export data=hpi_laspeyres 
			outfile="<Insert-Folder-Pathh>\hpi_lonmart_laspeyres.xls"
			replace;
run;

proc export data=hpi_paasche 
			outfile="<Insert-Folder-Path>\hpi_lonmart_paasche.xls"
			replace;
run;

proc export data=hpi_alltypes
			outfile="<Insert-Folder-Path>\hpi_lonmart_imputation.xls"
			replace;
run;
